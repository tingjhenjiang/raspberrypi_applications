...
EOM;
    ...
    function get_hamichs_htmlelements() {
        ...
        $this->links_elements = $docxpath->query("//div[@class='tvListBlock']/div[@class='list_item']//h3/a/@href");
    ...
    function get_channel_details() {
        ...
        foreach ($this->channels_elements as $key=>$channel) {
            ...
            // echo "\nhamilink is ".$hamilink;
            // $hamilink = preg_match("/\/channel\/(.+\.do)/", $hamilink, $hamilink_matches);
            // echo "\nhamilink_matches is: ";
            // dmpv($hamilink_matches);
            // $hamilink = $this->hamihost.$hamilink_matches[1];
            $hamilink = $this->hamihost.$hamilink;
            $hamilinks[] = $hamilink;
    ...
    function generate_hami_playlist() {
        ...
        foreach ($this->channels_elements as $key=>$channel) {
            ...
            $hamivideo_playlist_for_kodi .= str_replace("M3U8", "plugin://plugin.video.hamivideo/play/hami/".urlencode(basename($this->hamilinks[$key])), $tp)."\n";
